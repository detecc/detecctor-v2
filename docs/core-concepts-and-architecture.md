...
The plugin service does all the processing. It creates payloads and sends them to clients and if/when a client responds,
it processes the data sent back from the client. It then forwards the response to the Notification service if needed.

### 📲 Notification service

Notification service is essentially a bridge between various bots or other notification services and the end user. It
...
The management service manages user and client access to different plugins and commands. It interacts with the EMQX api
to manage the access to MQTT topics. Clients should only be able to communicate with the Management services and not
